...
import { BarChart3, CalendarDays, CircleHelp, ClipboardCheck, Clock3, Grid2X2, LogOut, Mail, Settings, UserRound, UsersRound } from "lucide-react";
...
const adminPrimary: readonly NavItem[] = [
  ["Dashboard", "/admin", Grid2X2], ["Messages", "/admin/messages", Mail], ["Teachers", "/admin/teachers", UsersRound], ["Students", "/admin/students", UserRound],
...
const teacherPrimary: readonly NavItem[] = [
  ["Dashboard", "/teacher", Grid2X2], ["Messages", "/teacher/messages", Mail], ["My Classes", "/teacher/classes", ClipboardCheck], ["Attendance", "/teacher/attendance", UsersRound],
...
export function Sidebar({ role, open, onClose }: { role: UserRole; open: boolean; onClose: () => void }) {
  const drawer = useDialog(open, onClose);
  ...
  const renderLinks = (items: readonly NavItem[]) => items.map(([label, href, Icon]) => {
    ...
    return <Link prefetch={false} key={href} href={href} className={active ? "active" : ""} aria-current={active ? "page" : undefined} onClick={onClose}>{active && <span className="active-indicator" aria-hidden="true" />}<Icon /><span>{label}</span></Link>;
  });
  return <>
    <button className={`drawer-scrim ${open ? "is-open" : ""}`} aria-label="Close menu" onClick={onClose} />
    <aside ref={drawer} tabIndex={-1} role={open ? "dialog" : undefined} aria-modal={open ? true : undefined} className={`sidebar ${open ? "is-open" : ""}`} aria-label={`${role === "ADMIN" ? "Administrator" : "Teacher"} navigation`}>
      <Link prefetch={false} href={role === "ADMIN" ? "/admin" : "/teacher"} className="brand" onClick={onClose}><XaadirLogo /></Link>
